public class Day2
{
   public static void main(String [] args)
   {
      ...
      {
         min=num1[j][0];//set min value
         max=num1[j][0];//set max value
      
         for(int i=0;i<num1[j].length;i++)
         {
            if(num1[j][i]<min)//find min number in each part of array
               min=num1[j][i];
            if(num1[j][i]>max)//find max number in each part of array
               max=num1[j][i];
         
         }
         dif+=max-min;//accumlate the difference 
      }
