...
public class Day2Q2
{
   public static void main(String [] args)
   {
      ...
      int first=0,second=0, dif=0;
      //Loop Y axis of array
      for(int x=0;x<num[0].length;x++)
      {
         //hold each number to check the rest
         for(int j=0;j<num[0].length;j++)
         {
            //loop through each number and check when divided by j that remainder is 0     
            for(int i=0;i<num[0].length;i++)
            {
               if(num[x][j] % num[x][i] == 0 && j!=i)
               {
                  first=num[x][j];
                  second=num[x][i];
         ...
         //find min of two numbers and divide bigger number by min number
         if(Math.min(first,second)==first)
         {
            dif+=second/first;
         }
         else
      ...
      System.out.print(dif);//output answer to console
